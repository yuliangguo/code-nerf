...
class Optimizer():
    ...
    def optimize_objs_w_pose(self, instance_ids, lr=1e-2, lr_half_interval=50, save_img=True, pose_mode=0):
        """
            ...
            pose_mode:
                0: Simplified camera model facing object center
                1: Euler angles
                2: Unit quaternion
        """
        logpath = os.path.join(self.save_dir, 'opt_hpams.json')
        ...
        for num_obj, d in enumerate(self.dataloader):
            ...
            while self.nopts < self.num_opts:
                ...
                for num, instance_id in enumerate(instance_ids):
                    ...
                    # ATTENTION: construct the graph from
                    rot_mat2opt = rot_trans.euler_angles_to_matrix(euler_angles2opt[num], 'XYZ')
                    pose2opt = torch.cat((rot_mat2opt, t_vec2opt[num].unsqueeze(-1)), dim=-1)
                    ...
                    # TODO: make the number of sampled rays a variable input
                    ...
                    optimized_poses.append(pose2opt)
                self.opts.step()
                self.log_opt_psnr_time(np.mean(loss_per_img), time.time() - t1, self.nopts + self.num_opts * num_obj,
                                       num_obj)
                self.log_regloss(reg_loss.item(), self.nopts, num_obj)
